...
<x-back-page-header title="Tag"></x-back-page-header>
                    ...
                    <table class="table table-bordered mb-0 table-centered">
                        ...
                        @forelse ($tags as $tag)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('tag.status',$tag->slug) }}" method="POST">
                                        @csrf
                                    <button type="submit" class="{{ ($tag->status == 'deactive') ? 'btn btn-danger btn-sm' : 'btn btn-success btn-sm' }}">{{ $tag->status }}</button>
                                    </form>
                                </td>
                                <td class="text-end">
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dLabel11" style="">
                                            <a class="dropdown-item" href="{{ route('tag.edit',$tag->id) }}"> <i class="ti ti-pencil"></i>  Edit</a>
